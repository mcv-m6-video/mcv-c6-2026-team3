...
class FCBottleneckLayers(nn.Module):

    def __init__(self, feat_dim, num_classes):
        super().__init__()
        self._fc_bottleneck1 = nn.Linear(feat_dim, feat_dim // 2)
        self._fc_bottleneck2 = nn.Linear(feat_dim // 2, feat_dim)
        self._fc_out = nn.Linear(feat_dim, num_classes)
        self.dropout = nn.Dropout()
    ...
    def forward(self, x):
        if len(x.shape) == 3:
            b, t, d = x.shape
            x = x.reshape(b * t, d)
            x = self.dropout(self._fc_bottleneck1(self.dropout(x)))
            x = self._fc_out(self._fc_bottleneck2(x))
