...
from model.model_spotting_unet import ModelUnet
from model.model_spotting_unet_loss import ModelUnetLoss
...
def update_args(args, config):
    ...
    args.use_soft_targets = config.get('use_soft_targets', False)
    args.soft_target_mode = config.get('soft_target_mode', 'fixed')
    args.soft_target_weights = config.get('soft_target_weights', [1.0, 0.5, 0.2])
    args.soft_target_sigma = config.get('soft_target_sigma', 1.0)
    args.soft_target_radius = config.get('soft_target_radius', 4)
    ...
    #Unet params
    args.use_unet = config.get("use_unet", False)
    args.dropout_unet = config.get("dropout_unet", 0.2)
    args.unet_kernel_size = config.get("unet_kernel_size", 3)
    args.unet_type = config.get("unet_type", "max")
    #This are multipliers over the original feature size comming from the backbone or custom
    args.unet_dims = config.get("unet_dims", [1, 2])
    args.custom_hidden = config.get("custom_hidden", False)
    if args.use_unet:
        print(f"USING UNET TYPE {args.unet_type}, with kernel size {args.unet_kernel_size}")
        print(f"HIDDEN DIMENSIONS ARE {args.unet_dims}")

    if args.use_soft_targets:
        if args.soft_target_mode == 'gaussian':
            print(
                'USING SOFT TARGETS: mode=gaussian '
                f'(sigma={args.soft_target_sigma}, radius={args.soft_target_radius})'
            )
        else:
            print(f"USING SOFT TARGETS: mode=fixed weights={args.soft_target_weights}")
...
def main(args):
    ...
    if args.clip_aug:
        ...
        model = ModelUnet(args=args)
        if args.use_soft_targets:
            model = ModelUnetLoss(args)
    else:
    ...
    # Evaluation on test split
    ap12_05, ap_score_05, _ = evaluate(model, test_data, nms_window = 5, delta=0.5)
    ap10_05 = compute_ap10(classes, ap_score_05)
    ...
    avg_table = [
        ["AP12", f"{ap12*100:.2f}", f"{ap12_05*100:.2f}"], 
        ["AP10", f"{ap10*100:.2f}", f"{ap10_05*100:.2f}"],
    ]
    
    headers = ["Average Precision", "Coarse (1s)", "Tight (0.5s)"]
